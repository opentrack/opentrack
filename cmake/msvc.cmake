...
set(opentrack-use-fast-math 0)
#set(opentrack-use-fast-math 1)
...
elseif(CMAKE_PROJECT_NAME STREQUAL "OpenCV")
    if(opentrack-use-fast-math)
        setq(ENABLE_FAST_MATH                   ON)
    else()
        setq(ENABLE_FAST_MATH                   OFF)
    endif()
    ...
    setq(WITH_OPENCL                            OFF)
    setq(WITH_OPENEXR                           OFF)
    setq(WIIH_OPENJPEG                          OFF)
    setq(WITH_PNG                               OFF)
    setq(WITH_PROTOBUF                          OFF)
    setq(WITH_TIFF                              OFF)
    setq(WITH_UNIFONT                           OFF)
    setq(WITH_VTK                               OFF)
    setq(WITH_WEBP                              OFF)
...
elseif(PROJECT_NAME STREQUAL "Qt")
    ...
    setq(BUILD_qtwebview                    OFF)
    setq(BUILD_qtquick3d                    OFF)
    setq(BUILD_qtwebengine_build            OFF)
    setq(BUILD_qtwebengine                  OFF)
    ...
    setq(FEATURE_clang                       ON)
    setq(FEATURE_clang_rtti                  ON)
elseif(PROJECT_NAME STREQUAL "LLVM")
    add_compile_options(-GL-)
    add_link_options(-LTCG:OFF)
    set(opentrack-use-fast-math 0)

    setq(BUILD_SHARED_LIBS                  OFF)
    setq(LLVM_ENABLE_PROJECTS           "clang")
    setq(LLVM_BUILD_UTILS                   OFF)
    setq(LLVM_ENABLE_BINDINGS               OFF)
    setq(LLVM_ENABLE_TELEMETRY              OFF)
    setq(LLVM_INCLUDE_BENCHMARKS            OFF)
    setq(LLVM_INCLUDE_DOCS                  OFF)
    setq(LLVM_INCLUDE_EXAMPLES              OFF)
    setq(LLVM_INCLUDE_TESTS                 OFF)
    #setq(LLVM_INCLUDE_TOOLS                 OFF)
    setq(LLVM_INCLUDE_UTILS                 OFF)
    setq(LIBCLANG_BUILD_STATIC               ON)
    setq(LLVM_TARGETS_TO_BUILD             host)
    setq(LLVM_ENABLE_RTTI                    ON)
endif()
...
if(opentrack-use-fast-math)
    set(_fast_math " -fp:fast")
else()
    set(_fast_math "")
endif()

setq(_CFLAGS                    "-diagnostics:caret -Zc:inline -Zc:preprocessor -Zc:__cplusplus -wd4117 -Zi -Zf -Zo -bigobj -vd0")
setq(_CFLAGS_RELEASE            "-O2 -Oit -Oy- -Ob3 -GS- -GF -GL -Gw -Gy${_fast_math}")
