...
bool pipeline::is_zero() const { QMutexLocker l(&b.lock); return !!(b.flags & f_zero); }
void pipeline::toggle_enabled() { b.negate(f_enabled_p); }
bool pipeline::is_enabled() const { QMutexLocker l(&b.lock); return !!(b.flags & f_enabled_p); }
