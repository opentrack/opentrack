...
struct Device
{
    ...
    QString guid;
    std::optional<GameInputDeviceInfo> info;
...
auto get_gi_create_ptr()
{
    ...
    static const gi_create funptr = []() {
        if (L.load())
        {
            auto p = L.resolve("GameInputCreate");
            if (p)
...
void Worker_::add_device(IGameInputDevice* dev)
{
    ...
    _devices[id] = Device{dev, make_device_guid(id), {}, {}};
    ...
    for (auto& [_, d] : _devices)
    {
        if (!d.info) [[unlikely]]
            if (const auto* ptr = d.dev->GetDeviceInfo())
                d.info = *ptr;
        const auto in = d.info ? d.info->supportedInput : GameInputKindUnknown;
...
Worker* make_gameinput_worker()
{
    ...
    if (!w->is_ok())
    {
        eval_once(qDebug() << "gi: not available");
        delete w;
        return nullptr;
    }
    else
    {
        //eval_once(qDebug() << "gi: ok");
        return w;
    }
}
